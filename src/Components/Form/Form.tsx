...
const Form = () => {
    ...
    const [brand, setBrand] = useState<{name: string; index?: number | undefined}>({
        name : 'Brands',
        index : 0
    });
    const [brandList, setBrandList] = useState<string[]>([]);

    const [model, setModel] = useState<{name: string; index?: number | undefined}>({
        name : 'Model',
        index : 1
    });
    ...
    const [engine, setEngine] = useState<{name: string; index?: number | undefined}>({
        name : 'Engine',
        index : 3
    });
    ...
    const changeHandler = (set : Dispatch<React.SetStateAction<{name: string; index?: number;}>>, value : string) => {
            ...
            <div className="w-10/12 bg-slate-200 p-6 rounded-lg flex flex-col shadow-md">
                    ...
                    <div className="flex flex-row justify-center gap-x-20 pt-10">
                        <div className="relative select-none" id="dropdownButton" ref={dropNum === 1 ? dropRef : null}>
                            ...
                            id="button" className="h-12 w-60 text-black rounded-sm text-xl font-primary outline 
                            outline-1 flex justify-between p-2 items-center cursor-pointer">
                                {brand.name}
                                <img src={downArrow} className="w-5 h-auto pl-2" />
                            </div>
                            {isOpen && dropNum === 1 ? <Dropdown data={brandList} DataFetch={Data} ObjectId={'65f046b17e9d5263e088681d'} nextState={setModelList}
                            set={setBrand} setIsOpen={setIsOpen} 
                            isOpen={isOpen} handler={changeHandler} resetState={resetState} index={brand.index} /> : null}
                    </div>
                    <div className="relative select-none" id="dropdownButton" ref={dropNum === 2 ? dropRef : null}>
                        <div onClick={() => {setIsOpen(!isOpen); setDropNum(2);}} 
                        id="button" className="h-12 w-60 text-black 
                        rounded-sm text-xl font-primary outline 
                        outline-1 flex justify-between p-2 items-center cursor-pointer">
                            {model.name}
                            <img src={downArrow} className="w-5 h-auto pl-2" />    
                        </div>
                        {isOpen && dropNum === 2 ? <Dropdown data={modelList} DataFetch={Data} ObjectId={'65fc073c0da0ac1bc6ed760a'} nextState={setGenList} 
                        set={setModel} setIsOpen={setIsOpen} 
                        isOpen={isOpen} handler={changeHandler} resetState={resetState} index={model.index}/> : null}
                    </div>
                    <div className="relative select-none" id="dropdownButton" ref={dropNum === 3 ? dropRef : null}>
                        <div onClick={() => {setIsOpen(!isOpen); setDropNum(3); }} id="button" className="h-12 w-60 text-black 
                        rounded-sm text-xl font-primary outline 
                        outline-1 flex justify-between p-2 items-center cursor-pointer">
                            {gen.name}
                            <img src={downArrow} className="w-5 h-auto pl-2" />
                        </div>
                        {isOpen && dropNum === 3 ? <Dropdown data={genList} DataFetch={false} ObjectId={'65fc073c0da0ac1bc6ed760a'} nextState={setEngineList}
                        set={setGen} setIsOpen={setIsOpen} 
                        isOpen={isOpen} handler={changeHandler} resetState={resetState} index={gen.index}/> : null}
                    </div>
                    <div className="relative select-none" id="dropdownButton" ref={dropNum === 4 ? dropRef : null}>
                        <div onClick={() => {setIsOpen(!isOpen); setDropNum(4);}} id="button" className="h-12 w-60 text-black 
                        rounded-sm text-xl font-primary outline 
                        outline-1 flex justify-between p-2 items-center cursor-pointer">
                            {engine.name}
                            <img src={downArrow} className="w-5 h-auto pl-2" />    
                        </div>
                        {isOpen && dropNum === 4 ? <Dropdown data={engineList} set={setEngine} setIsOpen={setIsOpen} 
                        isOpen={isOpen} handler={changeHandler} resetState={resetState} index={engine.index}/> : null}
                    </div>
                    </div>
                    {engine.name !== "Engine" ?
